...
public class CommandBind2 implements ICommand {

	private final Minecraft mc = Minecraft.getMinecraft();

	public Client BapeClient =null;

	public CommandBind2(Client BokerLite) {
		this.BapeClient = BokerLite;
	}
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		if (args.length == 2) {
			try {
				...
				if(Module == null){
					MessageUtils.send("L","Module not found. please use correct module name.", NotificationType.ERROR);
					// Helper.sendMessage("Module not found. please use correct module name.");
				} else {
					Module.setKey(Keyboard.getKeyIndex(key.toUpperCase()));
					MessageUtils.send(":)","bind "+Module.getName()+" to "+key, NotificationType.SUCCESS);
