...
import cn.BokerLite.utils.misc.Misc;
...
public class SubWindow extends Component {
    ...
    public void drawComponent(int mouseX, int mouseY, float partialTicks) {
        ...
        try {
            FontRenderer.F16.drawString(Client.chinese ? ch : title, posX + (width - FontRenderer.F16.getStringWidth(title)) / 2, posY + 8, -1);
        } catch (NullPointerException exception) {
            exception.printStackTrace();
        }
        FontRenderer.F16.drawString(hidden ? "+" : "-", posX + width - 10, posY + 8, -1);
        if (hidden)
            return;
    ...
    public int getWidth() {
        int width = FontRenderer.F16.getStringWidth(Client.chinese ? ch : title) + 32;
        for (Component component : components)
