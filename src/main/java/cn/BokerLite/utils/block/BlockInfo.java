package cn.BokerLite.utils.block;
...
public class BlockInfo {
    ...
    public static BlockInfo get(BlockPos blockPos) {
    	...
    	if (BlockUtil.canBeClicked(blockPos.add(0, -1, 0))) {
            ...
            return new BlockInfo(blockPos.add(0, 0, 1), EnumFacing.NORTH, null, rand.nextFloat(), 80f + rand.nextFloat());
        } else if (BlockUtil.canBeClicked(blockPos.add(-1, 0, 0))) {
            return new BlockInfo(blockPos.add(-1, 0, 0), EnumFacing.EAST, null, 90f + rand.nextFloat(), 80f + rand.nextFloat());
        } else if (BlockUtil.canBeClicked(blockPos.add(0, 0, -1))) {
            return new BlockInfo(blockPos.add(0, 0, -1), EnumFacing.SOUTH, null, -180f + rand.nextFloat() - 1, 80f + rand.nextFloat());
        } else if (BlockUtil.canBeClicked(blockPos.add(1, 0, 0))) {
            return new BlockInfo(blockPos.add(1, 0, 0), EnumFacing.WEST, null, -90f + rand.nextFloat(), 80f + rand.nextFloat());
        } else return null;
    }
}
